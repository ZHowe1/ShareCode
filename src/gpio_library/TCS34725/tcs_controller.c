 ...
 *
 * Description:: Code for controlling the TCS34725 RGB sensor
 *
 **************************************************************/
...
{
    char low = 0, high = 0;
    if (rgb_read_16(i2c, low_reg, &low) < 0) return -1;
    if (rgb_read_16(i2c, low_reg + 1, &high) < 0) return -1;
...
{
    if (i2c_set_target_address(0x29, i2c) < 0) {
        printf("Failed to set TCS I2C address.\n");
        return -1;
    }
...
rgb_color sense_color(i2c_data *i2c)
{
    ...
    if (read_channel_16(i2c, CLEAR_DATA, &color.clear) < 0) return color;
    if (read_channel_16(i2c, RED_DATA,   &color.red)   < 0) return color;
    if (read_channel_16(i2c, GREEN_DATA, &color.green) < 0) return color;
    if (read_channel_16(i2c, BLUE_DATA,  &color.blue)  < 0) return color;
